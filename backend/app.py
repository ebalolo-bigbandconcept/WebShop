...
from models import Parameters, TauxTVA, User, db, ma
from routes.admin import admin_bp
from routes.articles import articles_bp
from routes.auth import auth_alias_bp, auth_bp, limiter
...
ADMIN_MAIL = (
    open("/run/secrets/ADMIN_MAIL").read().strip()
    if os.path.exists("/run/secrets/ADMIN_MAIL")
    else os.getenv("ADMIN_MAIL")
)
ADMIN_PASSWORD = (
    open("/run/secrets/ADMIN_PASSWORD").read().strip()
    if os.path.exists("/run/secrets/ADMIN_PASSWORD")
    else os.getenv("ADMIN_PASSWORD")
)
FRONTEND_URL = os.getenv("FRONTEND_URL")

if not ADMIN_MAIL or not ADMIN_PASSWORD:
    logging.error(
        "ADMIN_MAIL et ADMIN_PASSWORD doivent être définis dans les variables d'environnement ou les secrets Docker."
    )
    raise ValueError(
        "ADMIN_MAIL et ADMIN_PASSWORD doivent être définis dans les variables d'environnement ou les secrets Docker."
    )
...
def enforce_csrf():
    ...
    token_request = request.headers.get("X-CSRF-Token") or request.cookies.get(
        "XSRF-TOKEN"
    )
...
def init_default_data():
    with app.app_context():
        try:
            ...
            logging.warning(
                f"Could not initialize default data (tables may not exist yet): {e}"
            )
            logging.info(
                "Run 'flask db upgrade' to create tables, then restart the app."
            )

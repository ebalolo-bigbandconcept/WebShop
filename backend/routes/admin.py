...
def modify_user(user_id):
    ...
    if user.role == "Administrateur":
        ...
        if admin_count <= 1 and new_role != "Administrateur":
            return (
                jsonify(
                    {
                        "error": "Impossible de modifier le rôle du dernier compte administrateur."
                    }
                ),
                403,
            )
    ...
    if user.role == "Administrateur":
        ...
        if user.id == current_user_id and new_role != "Administrateur":
            return (
                jsonify(
                    {
                        "error": "Impossible de modifier votre propre rôle administrateur."
                    }
                ),
                403,
            )
    ...
    error = validate_user_fields(
        new_email, new_first_name, new_last_name, new_password, new_role
    )
    if error:
        return jsonify({"error": error}), 400
...
def update_parameters():
    ...
    try:
        margin_rate = _coerce_float(
            body.get("marginRate")
            if body.get("marginRate") is not None
            else body.get("margin_rate")
        )
        margin_rate_location = _coerce_float(
            body.get("marginRateLocation")
            if body.get("marginRateLocation") is not None
            else body.get("margin_rate_location")
        )
        location_time = _coerce_int(
            body.get("locationTime")
            if body.get("locationTime") is not None
            else body.get("location_time")
        )
        location_subscription_cost = _coerce_float(
            body.get("locationSubscriptionCost")
            if body.get("locationSubscriptionCost") is not None
            else body.get("location_subscription_cost")
        )
        location_maintenance_cost = _coerce_float(
            (
                body.get("locationInterestsCost")
                if body.get("locationInterestsCost") is not None
                else body.get("location_maintenance_cost")
            )
        )
    ...
    general_conditions_sales = (
        body.get("generalConditionsSales")
        if body.get("generalConditionsSales") is not None
        else body.get("general_conditions_sales", "")
    ) or ""
    # Sanitize HTML to prevent XSS attacks - allow common HTML tags
    general_conditions_sales = bleach.clean(
        general_conditions_sales,
        tags=["b", "i", "u", "p", "br", "strong", "em", "ul", "ol", "li"],
        strip=True,
    )
    ...
    company_name = (
        ...
        else body.get("company_name", "")
    ) or ""
    company_address_line1 = (
        body.get("companyAddressLine1")
        if body.get("companyAddressLine1") is not None
        else body.get("company_address_line1", "")
    ) or ""
    company_address_line2 = (
        body.get("companyAddressLine2")
        if body.get("companyAddressLine2") is not None
        else body.get("company_address_line2", "")
    ) or ""
    company_zip = (
        body.get("companyZip")
        if body.get("companyZip") is not None
        else body.get("company_zip", "")
    ) or ""
    company_city = (
        body.get("companyCity")
        if body.get("companyCity") is not None
        else body.get("company_city", "")
    ) or ""
    company_phone = (
        body.get("companyPhone")
        if body.get("companyPhone") is not None
        else body.get("company_phone", "")
    ) or ""
    company_email = (
        body.get("companyEmail")
        if body.get("companyEmail") is not None
        else body.get("company_email", "")
    ) or ""
    company_iban = (
        body.get("companyIban")
        if body.get("companyIban") is not None
        else body.get("company_iban", "")
    ) or ""
    company_tva = (
        body.get("companyTva")
        if body.get("companyTva") is not None
        else body.get("company_tva", "")
    ) or ""
    company_siret = (
        body.get("companySiret")
        if body.get("companySiret") is not None
        else body.get("company_siret", "")
    ) or ""
    company_aprm = (
        body.get("companyAprm")
        if body.get("companyAprm") is not None
        else body.get("company_aprm", "")
    ) or ""

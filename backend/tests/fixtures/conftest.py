...
def admin_user(db_session, bcrypt_instance):
    ...
    """
    hashed_password = bcrypt_instance.generate_password_hash('AdminPass123!').decode('utf-8')
    admin = User(
        nom='Admin',
        prenom='Test',
...
def regular_user(db_session, bcrypt_instance):
    ...
    """
    hashed_password = bcrypt_instance.generate_password_hash('UserPass123!').decode('utf-8')
    user = User(
        nom='User',
        prenom='Test',
        email='user@test.com',
...
def test_devis(db_session, test_client_record, test_article, taux_tva_20):
    ...
    # Calculate totals
    devis_article.calculate_montant_ht()
    devis_article.calculate_montant_tva()
    devis_article.calculate_montant_ttc()
    
    devis.calculate_montant_ht()
    devis.calculate_montant_tva()
    devis.calculate_montant_ttc()

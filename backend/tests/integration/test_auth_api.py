...
class TestAuthLogin:
    ...
    def test_login_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/auth/login",
            json={"email": "test@test.com", "mdp": "TestPassword123!"},
        )
    ...
    def test_login_rate_limiting_protection(self, client):
        ...
        for i in range(6):
            response = client.post(
                "/api/auth/login",
                json={"email": "test@test.com", "mdp": "WrongPassword!"},
            )
...
class TestAuthLoginSuccess:
    ...
    def test_login_with_valid_credentials(self, client, regular_user):
        ...
        response = client.post(
            "/api/auth/login",
            json={"email": regular_user.email, "mdp": "TestPassword123!"},
        )
    ...
    def test_login_with_invalid_password(self, client, regular_user):
        ...
        response = client.post(
            "/api/auth/login",
            json={"email": regular_user.email, "mdp": "WrongPassword!"},
        )
    ...
    def test_login_with_nonexistent_user(self, client):
        ...
        response = client.post(
            "/api/auth/login",
            json={"email": "nonexistent@test.com", "mdp": "Password123!"},
        )

...
from models import Articles, Parameters, TauxTVA, User, db
...
class TestUserManagementExtended:
    ...
    def test_delete_user_nonexistent(self, client, admin_auth_headers):
        ...
        response = client.post(
            "/api/admin/delete-user/99999", headers=admin_auth_headers
        )
...
class TestParametersExtended:
    ...
    def test_update_parameters_with_invalid_numbers(
    ...
    def test_update_parameters_with_negative_values(
        self, client, admin_auth_headers, test_parameters
    ):
        """Test updating parameters with negative values"""
        response = client.post(
            "/api/admin/parameters",
            json={"marginRate": -1.0, "marginRateLocation": -0.5, "locationTime": -12},
            headers=admin_auth_headers,
        )
    ...
    def test_update_parameters_partial_update(
        self, client, admin_auth_headers, test_parameters
    ):
    ...
    def test_update_parameters_with_html_in_conditions(
        self, client, admin_auth_headers, test_parameters
    ):
...
class TestTVAManagementExtended:
    ...
    def test_create_tva_invalid_rate(self, client, admin_auth_headers):
        ...
        response = client.post(
            "/api/admin/tva", json={"taux": "not_a_number"}, headers=admin_auth_headers
        )
    ...
    def test_create_tva_negative_rate(self, client, admin_auth_headers):
        ...
        response = client.post(
            "/api/admin/tva", json={"taux": -0.20}, headers=admin_auth_headers
        )
    ...
    def test_create_tva_missing_rate(self, client, admin_auth_headers):
        ...
        response = client.post("/api/admin/tva", json={}, headers=admin_auth_headers)

        assert response.status_code == 400

    def test_delete_tva_nonexistent(self, client, admin_auth_headers):
        """Test deleting non-existent TVA returns 404"""
        response = client.delete("/api/admin/tva/99999", headers=admin_auth_headers)

        assert response.status_code == 404

    def test_delete_tva_in_use(
        self, client, admin_auth_headers, taux_tva_20, test_article
    ):
        """Test deleting TVA that's in use by articles"""
        response = client.delete(
            f"/api/admin/tva/{taux_tva_20.id}", headers=admin_auth_headers
        )

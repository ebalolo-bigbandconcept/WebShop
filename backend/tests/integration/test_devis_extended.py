...
import pytest
from models import db, Devis, DevisArticles, Parameters


class TestDevisRetrievalExtended:
    """Extended tests for devis retrieval endpoints."""
    
    def test_get_all_devis_pagination(self, client, auth_headers, test_client_record, test_article, taux_tva_20):
    ...
    def test_get_all_devis_page_boundaries(self, client, auth_headers):
        ...
        response = client.get('/api/devis/all?page=0&per_page=200', headers=auth_headers)
    ...
    def test_get_client_devis_multiple(self, client, auth_headers, test_client_record, taux_tva_20):
        ...
        response = client.get(f'/api/devis/client/{test_client_record.id}', headers=auth_headers)
        assert response.status_code == 200
        data = response.get_json()
        assert len(data.get('data', [])) >= 2
    
    def test_get_devis_info_signed_with_snapshot(self, client, auth_headers, test_devis):
...
class TestDevisUpdateExtended:
    ...
    def test_update_devis_status_to_signed(self, client, auth_headers, test_devis, test_article, taux_tva_20):
...
class TestDevisLocationCalculations:
    ...
    def test_devis_location_with_all_costs(self, client, auth_headers, test_client_record, test_article, taux_tva_20):
    ...
    def test_devis_location_zero_time(self, client, auth_headers, test_client_record, test_article, taux_tva_20):
...
class TestDevisScenarioExtended:
    ...
    def test_select_scenario_location_with_apport_extended(self, client, auth_headers, test_devis):
        ...
        assert response.status_code in [200, 400]  # May fail if devis not in proper state
...
class TestDevisRemiseCalculation:
    ...
    def test_create_devis_with_remise(self, client, auth_headers, test_client_record, test_article, taux_tva_20):
    ...
    def test_update_devis_increase_remise(self, client, auth_headers, test_devis, test_article, taux_tva_20):
...
class TestDevisArticleHandling:
    ...
    def test_devis_with_invalid_article_id(self, client, auth_headers, test_client_record):

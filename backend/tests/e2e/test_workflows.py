...
class TestDevisCreationWorkflow:
    ...
    def test_devis_creation_requires_auth(
...
class TestDevisPDFWorkflow:
    ...
    def test_devis_to_pdf_requires_auth(self, client, test_devis):
        ...
        response = client.get(
            f"/api/devis/pdf/{test_devis.id}", headers={"Accept": "application/pdf"}
        )
...
class TestAdminParameterImpactWorkflow:
    ...
    def test_margin_rate_parameter_impact(
        self, client, test_client_record, test_article
    ):
...
class TestSignedDevisImmutability:
    ...
    def test_signed_devis_immutability(self, client, signed_devis):
        ...
        delete_response = client.delete(f"/api/devis/delete/{signed_devis.id}")
...
class TestMultipleArticleDevis:
    ...
    def test_devis_with_multiple_articles_different_vat(
        self, client, test_client_record
    ):
        ...
        response = client.post(
            ...
            json={
                ...
                "is_location": False,
                "articles": [
                    {"article_id": 1, "quantite": 2, "taux_tva_id": 1},
                    {"article_id": 2, "quantite": 3, "taux_tva_id": 2},
                ],
            },
        )
